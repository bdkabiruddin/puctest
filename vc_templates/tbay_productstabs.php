		...
		<div class="tabs-container tab-heading text-center clearfix tab-v8">
			...
			<ul class="tabs-list nav nav-tabs">
				...
				<?php foreach ($list_query as $key => $li) { ?>
						<li <?php echo ($__count==0)?' class="active"':''; ?>><a href="#<?php echo esc_attr($key.'-'.$_id); ?>" data-toggle="tab" data-title="<?php echo esc_attr($li['title']);?>"><?php echo esc_html($li['title']);?></a></li>
					<?php $__count++; ?>
				<?php } ?>
			</ul>
		</div>
		...
		<?php if(  $layout_type == 'carousel' || $layout_type == 'carousel-special' ) { ?>
			...
			<div class="widget-content tab-content woocommerce">
				...
				<?php foreach ($list_query as $key => $li) { ?>
					...
					?>
					<div class="tab-pane animated fadeIn <?php echo ($__count == 0 ? ' active' : ''); ?>" id="<?php echo esc_attr($key).'-'.$_id; ?>">
